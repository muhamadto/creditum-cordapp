...
import static com.coffeebeans.creditum.TestUtils.CHARLIE;
...
/**
 * Practical exercise instructions for Contracts Part 3. The objective here is to write some contract code that verifies a transaction to settle an
 * [IOUState]. Settling is more complicated than transferring and issuing as it requires you to use multiple state types in a transaction. As with the
 * [IOUIssueTests] and [IOUTransferTests] uncomment each unit test and run them one at a time. Use the body of the tests and the task description to
 * determine how to get the tests to pass.
 */

public class IOUSettleTest {
  ...
  /**
   * Task 1. We need to add another case to deal with settling in the [IOUContract.verify] function. TODO: Add the [IOUContract.Commands.Settle] case
   * to the verify function. Hint: You can leave the body empty for now.
   */
  @Test
  ...
  /**
   * Task 2. For now, we only want to settle one IOU at once. We can use the [TransactionForContract.groupStates] function to group the IOUs by their
   * [linearId] property. We want to make sure there is only one group of input and output IOUs. TODO: Using [groupStates] add a constraint that
   * checks for one group of input/output IOUs. Hint: - The [groupStates] method on a Transaction takes two type parameters: the type of the state you
   * wish to group by and the type of the grouping key used (indicated by a method reference), in this case as you need to use the [linearId] and it
  ...
  /**
   * Task 3. There always has to be one input IOU in a settle transaction but there might not be an output IOU. TODO: Add a constraint to check there
   * is always one input IOU.
   */
  ...
  /**
   * Task 4. Now we need to ensure that there are cash states present in the outputs list. The [IOUContract] doesn't care about input cash as the
   * validity of the cash transaction will be checked by the [Cash] contract. We do however need to count how much cash is being used to settle and
   * update our [IOUState] accordingly. TODO: Filter out the cash states from the list of outputs list and assign them to a constant. Hint: - Use the
   * [outputsOfType] extension function to filter the transaction's outputs by type, in this case [Cash.State].
   */
  @Test
  ...
  /**
   * Task 5. Not only to we need to check that [Cash] output states are present but we need to check that the payer is correctly assigning the lender
   * as the new owner of these states. TODO: Add a constraint to check that the lender is the new owner of at least some output cash. Hint: - Not all
   * of the cash may be assigned to the lender as some of the input cash may be sent back to the borrower as change. - We need to use the
   * [Cash.State.getOwner()] method to check to see that it is the value of our public key. - Use [filter] to filter over the list of cash states to
   * get the ones which are being assigned to us. - Once we have this filtered list, we can sum the cash being paid to us so we know how much is being
   * settled.
   */
  @Test
  ...
  /**
   * Task 6. Now we need to sum the cash which is being assigned to the lender and compare this total against how much of the iou is left to pay.
   * TODO: Add a constraint that checks the lender cannot be paid more than the remaining IOU amount left to pay. Hint: - The remaining amount of the
   * IOU is the amount less the paid property. - To sum a list of [Cash.State]s, collect all [Cash.States] assigned to the lender and use a reduce
   * method or = explicitly loop through the list to sum the individual states. - We can compare the amount left paid to the amount being paid to use,
  ...
  /**
   * Task 7. Your Java implementation should handle this for you but it goes without saying that we should only be able to settle in the currency that
   * the IOU in denominated in. TODO: You shouldn't have anything to do here but here are some tests just to make sure!
   */
  @Test
  ...
  /**
   * Task 8. If we fully settle the IOU, then we are done and thus don't require one on ledgerServices.ledger anymore. However, if we only partially
   * settle the IOU, then we want to keep the IOU on ledger with an amended [paid] property. TODO: Write a constraint that ensures the correct
   * behaviour depending on the amount settled vs amount remaining. Hint: You can use a simple if statement and compare the total amount paid vs
  ...
  /**
   * Task 9. We want to make sure that the only property of the IOU which changes when we settle, is the paid amount. TODO: Write a constraint to
   * check only the paid property of the [IOUState] changes when settling.
   */
  @Test
  public void onlyPaidPropertyMayChange() {
    ...
    ledger(ledgerServices, l -> {
      ...
      l.transaction(tx -> {
        ...
        tx.input(Cash.class.getName(), fiveDollars);
        tx.output(Cash.class.getName(), fiveDollars.withNewOwner(ALICE.getParty()).getOwnableState());
        IOUState iouCopy = iou.copy(Currencies.DOLLARS(0), iou.getLender(), iou.getBorrower(), iou.getPaid()).pay(Currencies.DOLLARS(5));
        tx.output(IOUContract.IOU_CONTRACT_ID, iouCopy);
        tx.command(BOB.getPublicKey(), new Cash.Commands.Move());
        tx.command(Arrays.asList(BOB.getPublicKey(), ALICE.getPublicKey()), new IOUContract.Commands.Settle());
  ...
  /**
   * Task 10. Both the lender and the borrower must have signed an IOU issue transaction. TODO: Add a constraint to the contract code that ensures
   * this is the case.
   */
  @Test
