...
/**
 * This is the contract code which defines how the [IOUState] behaves. Looks at the unit tests in [IOUContractTests] for more insight on how this
 * contract verifies a transaction.
 */
...
public class IOUContract implements Contract {
  ...
  public static final String IOU_CONTRACT_ID = "com.coffeebeans.creditum.contract.IOUContract";

  /**
   * The IOUContract can handle three transaction types involving [IOUState]s. - Issuance: Issuing a new [IOUState] on the ledger, which is a
   * bilateral agreement between two parties. - Transfer: Re-assigning the lender/beneficiary. - Settle: Fully or partially settling the [IOUState]
   * using the Corda [Cash] contract.
   */
  ...
  /**
   * The contract code for the [IOUContract]. The constraints are self documenting so don't require any additional explanation.
   */
  @Override
  public void verify(LedgerTransaction tx) {
    ...
    if (commandData.equals(new Commands.Issue())) {
      ...
      requireThat(require -> {
        ...
        require.using("The lender and borrower cannot have the same identity.",
            outputState.getLender().getOwningKey() != outputState.getBorrower().getOwningKey());
        ...
        require.using("Both lender and borrower together only may sign IOU issue transaction.",
      ...
      requireThat(require -> {
        ...
        require.using("Only the lender property may change.",
            checkOutputState.getAmount().equals(inputState.getAmount()) && checkOutputState.getLinearId().equals(inputState.getLinearId())
                && checkOutputState.getBorrower().equals(inputState.getBorrower()) && checkOutputState.getPaid().equals(inputState.getPaid()));
        require.using("The lender property must change in a transfer.",
            !outputState.getLender().getOwningKey().equals(inputState.getLender().getOwningKey()));
        ...
        Set<PublicKey> listOfParticipantPublicKeys = inputState.getParticipants().stream().map(AbstractParty::getOwningKey)
        ...
        require.using("The borrower, old lender and new lender only must sign an IOU transfer transaction",
      ...
      requireThat(require -> {
        ...
        List<Cash.State> acceptableCash = allOutputCash.stream().filter(cash -> cash.getOwner().getOwningKey().equals(lenderIdentity.getOwningKey()))
        ...
        require.using("The amount settled cannot be more than the amount outstanding.",
            amountOutstanding.getQuantity() >= acceptableCashSum.getQuantity());
        ...
        List<PublicKey> arrayOfSigners = command.getSigners();
        Set<PublicKey> setOfSigners = new HashSet<PublicKey>(arrayOfSigners);
        require.using(" Both lender and borrower together only must sign IOU settle transaction.", setOfSigners.equals(listOfParticipantPublicKeys));
        return null;
      });
    }
  }
