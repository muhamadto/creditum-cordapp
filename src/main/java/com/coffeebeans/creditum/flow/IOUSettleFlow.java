package com.coffeebeans.creditum.flow;
...
public class IOUSettleFlow {
  ...
  /**
   * This is the flow which handles the settlement (partial or complete) of existing IOUs on the ledger. Gathering the counterparty's signature is
   * handled by the [CollectSignaturesFlow]. Notarisation (if required) and commitment to the ledger is handled by the [FinalityFlow]. The flow
   * returns the [SignedTransaction] that was committed to the ledger.
   */
  @InitiatingFlow
  @StartableByRPC
  public static class InitiatorFlow extends FlowLogic<SignedTransaction> {
    ...
    public SignedTransaction call() throws FlowException {
      ...
      } else if (amount.getQuantity() > (inputStateToSettle.getAmount().getQuantity() - inputStateToSettle.getPaid().getQuantity())) {
      ...
      CashUtils.generateSpend(getServiceHub(), tb, amount, getOurIdentityAndCert(), inputStateToSettle.getLender(), ImmutableSet.of()).getSecond();
  ...
  /**
   * This is the flow which signs IOU settlements. The signing is handled by the [SignTransactionFlow].
   */
  @InitiatedBy(com.coffeebeans.creditum.flow.IOUSettleFlow.InitiatorFlow.class)
  ...
  /**
   * Self issues the calling node an amount of cash in the desired currency. Only used for demo/sample/training purposes!
   */
